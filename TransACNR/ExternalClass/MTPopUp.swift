...
//        self.addGestureRecognizer(panGesture)
        ...
        lblMessage.font = UIFont(name: FontName.HelveticaNeueBold, size: 15)
...
//MARK: - Show Popup
    ...
    {
        var rectOld:CGRect = CGRect(x: view.frame.size.width/2 - 60 ,y: 0 ,width: 0 ,height: 0)
        var counter = 0
        var isFrameChange:Bool = false
        
        let colorAray:[UIColor] = [UIColor(red: 128.0/255.0, green: 158.0/255.0, blue: 73.0/255.0, alpha: 1) ,UIColor(red: 108.0/255.0, green: 108.0/255.0, blue: 108.0/255.0, alpha: 1)]
        ...
        for strBtnTitle in btnArray
        {
            ...
            let uicolor :UIColor  = colorAray[counter];
            
            
            counter += 1
            var widthLabel:CGFloat = ((btnArray.count == 1 && counter == 1) ? (120 ) : (((btnArray.count == 2 &&  counter == 2) || (btnArray.count == 2 && counter == 1)) ? 120 : 120))
            ...
            let rectNew:CGRect = CGRect(x: ((btnArray.count == 1  && counter == 1) ? (viewPopUp.frame.size.width / 2) - 60 : ((btnArray.count == 2 && counter == 2 && isFrameChange == false) ? (viewPopUp.frame.size.width / 2) + 10 : (viewPopUp.frame.size.width / 2) - 130)) ,y: rectOld.origin.y ,width: widthLabel,height: lblButtonNameDummy.requiredHeight() + 10)
    
            btnOk = UIButton(frame: rectNew)
            btnOk.backgroundColor = UIColor.TransMainBlue
