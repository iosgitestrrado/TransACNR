...
open class FrostedSidebar: UIViewController {
    ...
    */
    public init(itemImages: [String], colors: [UIColor]?, selectionStyle: SidebarItemSelectionStyle){
...
//            view.imageView.image = image
            ...
            view.imageView.sd_setImage(with: URL(string: image ), placeholderImage:UIImage(named: "imagePlaceHolder"))
    ...
    */
    open func showInViewController(_ viewController: UIViewController, animated: Bool){
        ...
        customView.frame = CGRect.init(x: 0, y: 0, width: 145, height: 170)
        customView.backgroundColor = UIColor(red:0.01, green:0.52, blue:0.88, alpha:1.00)
        ...
        imageView.frame = CGRect(x: 0, y: 0, width: 145, height: 125)
        customView.addSubview(imageView)
        let label = UILabel(frame: CGRect(x: 10, y: 125, width: 125, height: 45))
        label.textColor = .white
        label.numberOfLines = 0
        label.textAlignment = .center
        label.text = userDefaults.GET_USERDEFAULTS(objectValue:"BranchName")
    ...
    */
    ...
    fileprivate func layoutItems(){
        ...
        for (index, item) in itemViews.enumerated(){
            ...
            let frame = CGRect(x: leftPadding, y: 170 + topPadding*idx + itemSize.height*idx + topPadding, width:itemSize.width, height: itemSize.height)
            item.frame = frame
        ...
        if adjustForNavigationBar{
            contentView.contentSize = CGSize(width: 0, height: (itemCount + 0.5) * (itemSize.height + topPadding) + topPadding + 170)
        } else {
            contentView.contentSize = CGSize(width: 0, height: itemCount * (itemSize.height + topPadding) + topPadding + 170)
        }
    }
